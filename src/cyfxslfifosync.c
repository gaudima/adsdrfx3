...
//#include "cyfxgpif2configNewC2.h"
	...
	if( bRequest == 0x05)
	{
		...
		//if (bType == CY_U3P_USB_VENDOR_RQT)
		{
	   		...
	   		}else if (bRequest == 0xB5)
	   		{
				//Old recieve function
	   			...
	   			//if ((bReqType & 0x80) == 0)
	   			{
	   				//CyU3PUsbGetEP0Data (wLength, glEp0Buffer, NULL);
	   					//SPI send
	   				glEp0Buffer[2] = wValue; glEp0Buffer[1] = (wIndex&0xff); glEp0Buffer[0] = (wIndex>>8);
	   		...
	   		}else if (bRequest == 0xBB)
	   		{
                ...
                CyU3PGpioSetValue(ADSPI_EN, CyFalse);
                glEp0MultipleDataBuffer[32] = CyU3PSpiTransmitReceiveWords(glEp0MultipleDataBuffer, wLength);
                CyU3PGpioSetValue(ADSPI_EN, CyTrue);
                return CyTrue;
